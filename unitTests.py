...
def unitTests():
    ...
    assert np.allclose(Zhat[-1, :], [ 1.50067515, 0.04742477], atol=absolute_tolerance)
    assert np.allclose(params['A'][0, :], [ 0.66884415, -0.17173555], atol=absolute_tolerance)
    ...
    assert np.allclose(Zhat[-1, :], [1.49712162, 0.05823952], atol=absolute_tolerance) # this is slightly different (though highly correlated) because ZIFA runs one extra half-step of EM
    assert np.allclose(params['A'][0, :], [ 0.66884415, -0.17173555], atol=absolute_tolerance)
    assert np.allclose(params['decay_coef'], 0.10458794970222711, atol=absolute_tolerance)
    assert np.allclose(params['sigmas'][0], 0.30219903, atol=absolute_tolerance)

    Zhat, params = block_ZIFA.fitModel(Y, k, n_blocks = 3)
    assert np.allclose(Zhat[-1, :], [  9.84455438e-01, 4.50924335e-02], atol=absolute_tolerance)
    ...
    X, Y, Z, ids = generateSimulatedDimensionalityReductionData(n_clusters, n, d, k, sigma, decay_coef)
    Zhat, params = block_ZIFA.fitModel(Y, k, n_blocks = 3)
    assert np.allclose(Zhat[-1, :], [-1.69609638,-0.5475882, 0.08008015], atol=absolute_tolerance)

    X, Y, Z, ids = generateSimulatedDimensionalityReductionData(n_clusters, n, d, k, sigma, decay_coef)
    Zhat, params = ZIFA.fitModel(Y, k)
    print(Zhat[-1, :])
    assert np.allclose(Zhat[-1, :], [-0.63075905, -0.77361427, -0.11544281], atol=absolute_tolerance)
